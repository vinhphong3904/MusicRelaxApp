...
//http://localhost:8080/play/noi-nay-co-anh.mp3 để chạy
...
const { getTopSongs,getRecommendSongsForGuest, getRecommendSongsForUser} = require('./dbOperations');
...
//Bài hát đề xuất dựa trên user hay guest
app.get('/api/songs/recommend', authOptional, async (req, res) => {
  try {
    //GUEST
    if (!req.user) {
      const songs = await getRecommendSongsForGuest(10);
      return res.json({
        success: true,
        type: 'GUEST',
        data: songs
    ...
    //USER
    const songs = await getRecommendSongsForUser(req.user.id, 10);
    res.json({
      success: true,
      type: 'USER',
      data: songs
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to get recommend songs'
    });
  }
});
...
// ---------- GENRES ----------
app.get('/api/genres', authenticateToken, asyncHandler(async (req, res) => {
  const genres = await db.getAllGenres();
  return res.json({ success: true, data: genres });
}));

app.get('/api/genres/:id', authenticateToken,
...
app.get(
  ...
  authenticateToken,
  asyncHandler(async (req, res) => {
    const result = await db.getAllAlbums();
    return res.json(result);
  })
);

app.get('/api/albums/:id', authenticateToken,
  validate([ param('id').isInt() ]),
  asyncHandler(async (req, res) => {
    const id = parseInt(req.params.id);
